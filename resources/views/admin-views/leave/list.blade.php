...
@section('title', translate('Leave_Request_list'))
    ...
    <div class="content container-fluid">
            ...
            <div class="row align-items-center">
                <div class="col-sm">
                    <h1 class="page-header-title">
                        ...
                        {{ translate('messages.Leave_Request_logs') }}
                    </h1>
                </div>
            </div>
            <!-- End Row -->
        ...
        <div class="card">
            ...
            <div class="card-header gap-2 flex-wrap pt-3 border-0">
                <h3 class="m-0">
                    {{ translate('messages.Leave_Request_logs') }} <span class="badge badge-soft-dark ml-2"
                        id="count">{{ $leave_request_logs->total() }}</span>
                </h3>
            ...
            <div class="table-responsive datatable-custom">
                <table id="datatable"
                    ...
                    @endphp
                    <tbody id="set-rows">
                        @foreach ($leave_request_logs as $key => $leave_request_log)
                            <tr class="">
                                <td class="">
                                    {{ (request()->get('show_limit') ? $count++ : $key) + $leave_request_logs->firstItem() }}
                                </td>
                                <td class="table-column-pl-0">
                                    <div class="d-flex align-items-center gap-2">
                                        <span class="text-body text-hover-primary">
                                            {{  $leave_request_log->employee->f_name?  $leave_request_log->employee->f_name . ' ' . $leave_request_log->employee->l_name : translate('Staff_not_found') }}
                                        </span>
                                    </div>
                                </td>
                                <td>
                                    ...
                                    @elseif($leave_request_log->leave_type == 'sl')
                                        Sick Leave
                                    @else
                                        {{ $leave_request_log->leave_type }}
                                    @endif
                                </td>
                                <td>
                                    <label class="badge">
                                        {{ \App\CentralLogics\Helpers::date_format($leave_request_log->from_date) }}
                                    </label>
                                </td>
                                <td>
                                    <label class="badge">
                                    {{ \App\CentralLogics\Helpers::date_format($leave_request_log->to_date) }}
                                    </label>
                                </td>                
                                <td>
                                ...
                                <td>
                                @if(($leave_request_log->leave_status == 'pending') || ($leave_request_log->leave_status == 'rejected'))
                                    <!-- Approve Button -->
                                    <form action="{{ route('admin.leave-request.status', ['id' => $leave_request_log->id, 'leave_status' => 'approved']) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('POST')
                                ...
                                @endif
                                @if(($leave_request_log->leave_status == 'pending') || ($leave_request_log->leave_status == 'approved'))
                                    <!-- Reject Button -->
                                    <form action="{{ route('admin.leave-request.status', ['id' => $leave_request_log->id, 'leave_status' => 'rejected']) }}" method="POST" style="display:inline;">
                                        @csrf
                                        @method('POST')
            ...
            <div class="page-area px-4 pb-3">
                <div class="d-flex align-items-center justify-content-end">
                    <div>
                        {!! $leave_request_logs->withQueryString()->links() !!}
                    </div>
                </div>
            </div>
            <!-- End Footer -->
