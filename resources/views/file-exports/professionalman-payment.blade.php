<div class="row">
    <div class="col-lg-12 text-center "><h1 >{{ translate('messages.Professionalman_Payments') }}</h1></div>
    <div class="col-lg-12">
    ...
    <table>
        <thead>
            ...
            <tr>
                ...
                <th>{{ translate('messages.Professionalman_Name') }}</th>
        ...
        @foreach($data['dm_earnings'] as $key => $at)
            <tr>
                ...
                <td>
                    @if($at->delivery_man)
                    {{$at->professional_man->f_name.' '.$at->professional_man->l_name}}
                    @else
                    {{translate('messages.professionalman_deleted')}}
                    @endif
                </td>
                <td>
                    @if($at->professional_man)
                    {{$at->professional_man->phone}}
                    @else
                    {{translate('messages.professionalman_deleted')}}
                    @endif
                </td>
